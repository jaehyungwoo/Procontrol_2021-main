...
if C_n(1)~=C_n(2) && max(C_n)>threshold && response_flag==0
    ...
    marker_color_array(n,:) = choice_colors((sign(obtainable(output))+2),:);
...
if response_flag
    if t>outcome_time
        ...
        % set valence and risk_learn parameter 'theta'
        give_feedback;    %
...
if seed; rng(seed*10000+n*1000+t); end     % set unique random seed for every trial & time point
